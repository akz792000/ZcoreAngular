...
$.extend(CalendarsPicker.prototype, {
	...
	_popupClass: 'calendars-popup ui calendar', // Marker for popup division
	...
	defaultRenderer: {
		...
		'<div class="calendars-nav ui calendar header top">{link:prev}{link:today}{link:next}</div>{months}' +
		'{popup:start}<div class="calendars-ctrl ui calendar header bottom">{link:clear}{link:close}</div>{popup:end}' +
		...
		month: '<div class="calendars-month ui header"><div class="calendars-month-header">{monthHeader}</div>' +
		'<table class="ui"><thead>{weekHeader}</thead><tbody>{weeks}</tbody></table></div>',
	...
	_checkExternalClick: function(event) {
		...
		if (!target.parents().addBack().hasClass($.calendars.picker._popupClass) &&
				!target.hasClass($.calendars.picker.markerClass) &&
				!target.parents().addBack().hasClass($.calendars.picker._triggerClass)) {
